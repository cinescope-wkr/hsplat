...
try:
    ...
    if cuda_toolkit_available():
        ...
        # extra_include_paths.append(glm_path)
        extra_cflags = ["-O3"]
        if NO_FAST_MATH:
        ...
        if os.path.exists(os.path.join(build_dir, "hsplat_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "hsplat_cuda.lib")
        ):
            ...
            with Console().status(
                f"[bold yellow]hsplat_cuda: Setting up CUDA with MAX_JOBS={os.environ['MAX_JOBS']} (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]hsplat_cuda: No CUDA toolkit found. hsplat_cuda will be disabled.[/yellow]"
        )
